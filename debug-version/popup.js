...
swipperBox.addEventListener('click', (e) => {   
  if (e.target.classList.contains('darkbtn')) {
    ...
    console.log(window.allLinkData[mySwiper.realIndex].links)
...
function loadSavedData(response) {
  ...
  console.log('from load-group', response)
  Object.keys(response).forEach((e, i) => {
    var listHtml=''
    var newSlide=''
    console.log(response[e].title, response[e].links)
